...
function synonyms(content){
    async function createStreamChat(
        ...
        const synonyms = tmp2.map(word => `${word.replace(/^[^a-zA-Z0-9가-힣]+/, '').trim()}`);
        const contentArray = content.split(',').map(word => word.trim());
        filteredSynonyms = synonyms.filter(synonyms => !contentArray.includes(synonyms));
        filteredSynonyms = filteredSynonyms.filter(synonyms => synonyms !== '');
        console.log(filteredSynonyms);
